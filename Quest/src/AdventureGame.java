...
public class AdventureGame implements KeyListener,MouseListener {
	...
	public static void main(String[] args) {
		...
		JFrame frame2 = new JFrame();				//new frame object
		frame2.setSize(100,100);					//the size of the new frame
		JButton button = new JButton("Music");		//create a button object	
		frame2.add(button);							//add the button to frame2
		button.addActionListener(new listener());	//add listener for event to the button
		frame2.show(true);							//show frame2 window
	}
	
	private static class listener implements ActionListener{	
		public final void action (ActionEvent e){
			music();								//play music when the button is clicked
		}
	}
	
	private static void music() {
		AudioPlayer audio = AudioPlayer.player;		//create audio object
		AudioStream music;							//
		AudioData quest;							//
		ContinuousAudioDataStream cont = null;		//to continuously playing the audio file
		
		try{
			music = new AudioStream(new FileInputStream("Quest.mp3"));	//to open the audio file
			quest = music.getData();		//get the data from the audio file
			cont = new ContinuousAudioDataStream(quest);		//to play the audio file on repeat
		} catch (IOException error) {} //catch IOException
		
		audio.start(cont);	//start the audio file and 
	}
